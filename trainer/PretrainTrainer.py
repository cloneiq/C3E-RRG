...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        # 如果模型类型是 'mae' (Masked Autoencoder)，则执行 mae_train_epoch方法
    ...
    def select_task(self):
        ...
        if selection <= 0.5:
            ...
            return "mim" # Masked Image Modeling（掩码图像建模）
        else:
            return "plm" # Prefix Language Modeling（前缀语言建模）
    ...
    def mae_train_epoch(self, epoch):
        ...
        for batch_idx, (images_id, images, reports_ids, reports_masks) in enumerate(self.train_dataloader):
            B = images.size(0)
            _image = torch.zeros_like(images[:, 0]).cuda()  # 创建与图像相同形状的零张量并移到GPU
            # 为每个样本随机选择一个图像（50%概率选择第一张或第二张）
            for i in range(B):
                image_idx = 0
                if torch.rand(1) > 0.5:
                    image_idx = 1
            ...
            # 前向传播：计算损失、预测结果和掩码
            loss, pred, mask = self.model(_image) # 清空梯度
            self.optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_value_(self.model.parameters(), 0.1)  # 梯度裁剪，防止梯度爆炸
            self.optimizer.step()

            train_v_loss += loss.item()

            # record loss per iter记录每个迭代的损失
            self.monitor.record_loss(epoch, batch_idx, len(self.train_dataloader), [loss.item()])

            # 如果不是StepLR调度器，每个批次后更新学习率
            if self.args.lr_scheduler != 'StepLR':
                self.lr_scheduler.step()
        if self.args.lr_scheduler == 'StepLR':
            self.lr_scheduler.step()
    ...
    def test_epoch(self, mode='img'):
        ...
        with torch.no_grad():
            for batch_idx, (images_id, images, reports_ids, reports_masks) in enumerate(self.test_dataloader):
                ...
                if self.args.model == 'mae': # Masked Autoencoder
                    image = images[:, 1]
    ...
    def vis_img(self, output_img, image, mask, ep):
        """
        visualize the recon images, all input is tensor，可视化重建图像
        ...
        _img = OrderedDict()
        for i in range(n):
            # 处理输出图像：分离、转换为float、移到CPU、转换为PIL图像
    ...
    def vis_report(self, outputs, image, target, ep):
        """
        visualize the output report, all input is tensor，可视化输出报告，所有输入都是张量
