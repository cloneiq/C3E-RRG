...
class BaseTrainer(object): # 管理模型训练的通用流程
    def __init__(self, model, criterion, metric_ftns, optimizer, args, monitor=True):
        ...
        self.save_period = self.args["save_period"] # 模型保存间隔

        self.mnt_mode = args["monitor_mode"]    # 监控模式：min(最小化)或max(最大化)
        self.mnt_metric = 'val_' + args["monitor_metric"] # 验证集监控指标
        ...
        self.start_epoch = 1
        self.checkpoint_dir = args["result_dir"]  # 检查点保存目录
        ...
        # monitor
        if monitor:
            self.monitor = Monitor(args)   # 创建训练监控器
    ...
    def train(self):
        not_improved_count = 0  # 记录性能未提升的连续epoch数
        for epoch in range(self.start_epoch, self.epochs + 1):  # 抽象方法，由子类实现具体训练逻辑
            result = self._train_epoch(epoch)
            if result is None:
                self._save_checkpoint(epoch) # 如果没有结果，直接保存检查点
                continue
            ...
            if self.mnt_mode != 'off': # 监控模式开启
                try:
                    ...
                    print("Warning: Metric '{}' is not found. " "Model performance monitoring is disabled.".format(
                        self.mnt_metric))
                    self.mnt_mode = 'off' # 关闭监控
                    improved = False

                if improved:
                    self.mnt_best = log[self.mnt_metric]
                    not_improved_count = 0 # 重置未提升计数
                    best = True
                else:
                    not_improved_count += 1   # 增加未提升计数

                # 早停机制：如果连续多个epoch性能未提升，停止训练
...
class Trainer(BaseTrainer):
    ...
    def inference(self):
        ...
        with torch.no_grad():
            test_gts, test_res = [], []
            sub_ids, stu_ids = [], []  # <<<<<< 新增：收集 subject_id / study_id
            p = torch.zeros([1, self.args["max_seq_length"]]).cuda()
            ...
            for batch_idx, (images_id, images, reports_ids, reports_masks) in enumerate(self.test_dataloader):
                ...
                test_res.extend(reports)
                test_gts.extend(ground_truths)
                # <<<<<< 新增：把 images_id 里的 (subject_id, study_id) 拆出来
                # mimic: images_id 一般是 list，元素是 (sub, stu)
