...
class MimiccxrSingleImageDataset(BaseDataset):
    def __getitem__(self, idx):
        example = self.examples[idx]
        # image_id = example['id']
        # ---- mimic: return (subject_id, study_id) ----
        sub = example.get("subject_id", None)
        stu = example.get("study_id", None)
        if sub is not None and stu is not None:
            image_id = (int(sub), int(stu))
        else:
            # fallback: still keep uuid if meta missing
            image_id = example["id"]
        image_path = example['image_path']
...
class MixSingleImageDataset(Dataset):
    def __init__(self, args, tokenizer, split, transform=None):
        self.image_dir = {'iu_xray': 'data/iu_xray/images/',
                          'mimic_cxr': 'data/mimic_cxr/images/'}
    ...
    def __getitem__(self, idx):
        example = self.examples[idx]
        # image_id = example['id']
        # ---- mix: IU 用原 id；mimic 返回 (subject_id, study_id) ----
        if len(example["image_path"]) == 1:  # mimic
            sub = example.get("subject_id", None)
            stu = example.get("study_id", None)
            if sub is not None and stu is not None:
                image_id = (int(sub), int(stu))
